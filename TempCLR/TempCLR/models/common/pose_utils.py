...
from TempCLR.utils import (batch_rodrigues, batch_rot2aa, Tensor)
...
def build_pose_decoder(cfg, num_angles, mean_pose=None, pca_basis=None):
    ...
    if param_type == 'aa':
        ...
        decoder = ClusterDecoder(num_angles=num_angles, **cfg)
    else:
...
class ClusterDecoder(nn.Module):
    def __init__(self, data_fn='', num_angles=1, tau=1,
                 dtype=torch.float32, **kwargs):
        '''
            Parameters
            ----------
        ...
        logger.debug('Loading {}', data_fn)
        self.dtype = dtype
        self.tau = tau

        data_fn = osp.expandvars(data_fn)
        with open(data_fn, 'rb') as data_file:
            data = pickle.load(data_file)

        self.rot_repr = data['rot_repr']
        self.num_clusters = data['num_clusters']
        self.euler_order = data['euler_order']
        self.num_angles = num_angles

        centroids = data['centroids'][:, :num_angles]
        self.dim = centroids.shape[-1]
        # 1xKxJxPx1
        centroids = torch.tensor(
            centroids, dtype=dtype).unsqueeze(dim=0).unsqueeze(dim=-1)
        centroids = centroids.permute([0, 2, 3, 4, 1])
        logger.debug('Centroids shape: {}', centroids.shape)
        self.register_buffer('centroids', centroids)

        self.dim_size = (self.num_clusters * self.num_angles)

        if self.rot_repr == 'cont-repr' or self.rot_repr == 'cont_rot_repr':
            self.decoder = ContinuousRotReprDecoder(num_angles, dtype=dtype)
        elif self.rot_repr == 'euler':
            self.decoder = EulerDecoder(num_angles, dtype=dtype)
        else:
            raise NotImplementedError
    ...
    def extra_repr(self):
        msg = 'Rotation representation: {}\n'.format(self.rot_repr)
        msg += 'Number of Clusters: {}\n'.format(self.num_clusters)
        if self.rot_repr == 'euler':
            msg += 'Euler order: {}'.format(self.euler_order)
        return msg
    ...
    def get_mean(self):
        return torch.full([self.dim_size], -9e15, dtype=self.dtype)

    def forward(self, decoder_input):
        '''
        '''
        weights = decoder_input.view(decoder_input.shape[0],
                                     self.num_clusters,
                                     self.num_angles,
                                     -1)
        # Should be BxNUM_CLUSTERSxJx3
        exp_weights = torch.exp(weights / self.tau)
        norm_factor = torch.sum(exp_weights, dim=1)

        centroids = self.centroids.expand(weights.shape[0], -1, -1, -1)

        output = torch.sum(centroids * exp_weights, dim=1) / norm_factor
...
def build_pose_parameterization(
    ...
    if type == 'aa':
        ...
        decoder = ClusterDecoder(num_angles=num_angles, **kwargs)
        dim = decoder.get_dim_size()
        mean = decoder.get_mean()
        ind_dim = decoder.num_clusters
    else:
